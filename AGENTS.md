...
- Every PR gets complementary independent reviews. Request `copilot-pull-request-reviewer` after the PR exists; if `gh pr view` is ambiguous, verify requested-reviewer state through the GitHub API. Run the local Claude review yourself rather than requesting a Claude or other bot account as a GitHub reviewer. For an existing PR, use `claude -p "/code-review max <PR#> --comment" --model opus --effort max` when the CLI and external review action are available and authorized. If that lane is unavailable, stop and report the review-gate blocker; do not substitute another reviewer without an explicit user waiver.
- Wait for every requested review lane and all repository-mandated checks on the current head. Address every actionable comment and unresolved review thread on the same PR. Before every push, rerun the repository-mandated local validation for that exact head as defined by the pre-push section above. After the push, refresh reviews and checks for the new head and rerun affected smoke lanes. A behavior-affecting push invalidates smoke evidence from an older head.
