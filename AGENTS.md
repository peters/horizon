...
- Drive the isolated application through explicitly configured device CLI/MCP targets; native automation, when needed, must be scoped to the fixture display and exact PID. The Device panel is read-only. Browser-page interaction remains on the `horizon-browser` skill's public `browser_*` tools; `device_panel` manages native viewers, not browser pages.
