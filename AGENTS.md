...
### Isolated UI Testing Through noVNC

- All future interactive Horizon UI testing must run in a task-owned isolated desktop viewed through **noVNC**. Never launch or drive the test instance on the developer's active desktop, steal its focus, or reuse an existing Horizon session. Unit tests, headless integration tests and static checks do not need a viewer.
- Run parallel application scenarios on distinct task-owned displays, control targets, ports, private state and evidence directories. Freeze candidate executables before launching from a shared build cache and verify the actual application child PID and executable hash, not just a sandbox wrapper.
...
- On Linux, use the [local Horizon smoke fixture](scripts/device-smoke/README.md#horizon-inside-a-novnc-browser-panel): a separate Xvfb display and window manager, private application state, a loopback VNC server and noVNC. noVNC is the viewer; the separate desktop and private state provide isolation. Mirroring the developer's desktop through VNC does not satisfy this rule.
- Open the fixture's noVNC URL in a separate Horizon browser panel. Browser interaction must use the `horizon-browser` skill and its public `browser_*` MCP tools. Drive the nested test application through device CLI/MCP or native automation explicitly scoped to the fixture display and exact PID. If the browser MCP server is unavailable, report that it is not connected; do not switch browser controllers or fall back to the developer's desktop.
- For visible feature additions or behavior changes, capture a short video of the exercised flow from the isolated desktop or its noVNC view, plus screenshots after launch and resize/fit. Reuse the public `browser_video` tool when available; noVNC alone does not create a recording. Start recording before the interaction, stop afterward, and play back or decode representative frames to verify that the feature and movement were actually captured. Copy the finalized export into the private evidence directory before closing its browser panel, which removes the capture profile. Keep evidence local until publication is authorized, and record the candidate binary/commit and scenario with it.
- On macOS or Windows, use a dedicated test machine, VM or isolated desktop session exposed through noVNC; never commandeer the developer's active session. Platform-specific graphics/input checks must still run on the actual target OS. If a suitable isolated environment is unavailable, report the blocked lane rather than silently using the active desktop.
- Close the exact test window normally and clean up only fixture-owned processes and state. Closing the viewer is not proof that the application exited. Preserve the developer's Horizon, terminals and desktop throughout the test.
...
- When a bug depends on live movement, capture a short video from the isolated test desktop/noVNC view; use high-frequency native window position traces for precise movement assertions and screenshots as supporting evidence. Scope any native recorder to the test desktop, never the developer's active desktop.
...
- Use the task-owned virtual display and lightweight window manager described in the noVNC testing rule above so focus and hotkey automation cannot interfere with the user's desktop. Allocate an unused display atomically, and stop only the display and window-manager processes created for the test
