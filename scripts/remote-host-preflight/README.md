...
| `container_engine` | `docker version --format json` / `docker context inspect --format '{{.Endpoints.docker.Host}}'` / `docker info --format '{{.Driver}}'` / `podman --remote=false info --format '{{.Version.Version}}'` | a usable engine is present **and reachable by the current user on this host**: the endpoint must be a local unix socket (`DOCKER_HOST` or the active docker context Host; named podman/`CONTAINER_*` connections and `containers.conf` `engine.remote` are rejected — podman is forced local with `--remote=false` and is queried only when a local API socket already exists, so `podman info` cannot initialize rootless runtime state), the docker server OS must be present and `linux`, and for docker the storage driver is reported or explicitly marked `unverified` with the reason. Engine probes request only those fields |
