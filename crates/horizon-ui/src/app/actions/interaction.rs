...
impl HorizonApp {
    ...
    fn pointer_over_native_select_popup(
        &self,
        ctx: &Context,
        visible_panel_ids: impl IntoIterator<Item = PanelId>,
        position: egui::Pos2,
    ) -> bool {
        let viewport = ctx.viewport_id();
        visible_panel_ids.into_iter().any(|id| {
            self.panel_render_caches
                .browser_ui_state
                .get(&id)
                .is_some_and(|state| state.select_menu_contains(viewport, position))
        })
    }
