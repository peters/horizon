...
//! viewport follows the panel, so the board resize is the resize; the host
//! then stamps the resulting size on the panel's manifest before the result
//! is published, so the caller can verify the size it asked for.
...
use super::browser_requests::{ActorPanel, actor_panel, browser_workspace, launched_by_this_host, viewport_from_size};
...
impl HorizonApp {
    ...
    pub(super) fn resize_target(
        ...
        Ok((panel_id, [request.width as f32, request.height as f32]))
    }
    ...
    fn apply_browser_resize_request(&mut self, request: &BrowserResizeRequest, actor_panel: ActorPanel) -> bool {
        ...
        if let Err(error) = manifest::sync_host_state_in(
            ...
            panel.visible,
            &workspace,
            Some(viewport_from_size(panel.layout.size)),
        ) {
        ...
        complete_resize_result(&root, &BrowserResizeResult::ready(request));
        if resized {
...
mod tests {
    ...
    fn a_claimed_resize_applies_the_size_and_publishes_the_result() {
        ...
        assert!(
            approx_size(panel.layout.size, [1920.0, 1080.0]),
            "panel resized to the requested size"
        );
    ...
    fn refusals_keep_the_panel_and_name_the_reason() {
        ...
        assert!(approx_size(size, [640.0, 480.0]));
