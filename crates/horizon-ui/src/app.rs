                ...
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        ...
                        for (ws_id, ws_name, ws_color, is_active, panel_ids) in &ws_data {
                            ...
                            let ws_color = *ws_color;
                            let is_active = *is_active;
                            let ws_attention_count = panel_ids
                                .iter()
                                .filter(|pid| self.board.unresolved_attention_for_panel(**pid).is_some())
                                .count();
                            ...
                            let ws_response = ui.horizontal(|ui| {
                                ...
                                // Accent bar — red if any panel needs attention
                                let bar_color = if ws_attention_count > 0 {
                                    theme::PALETTE_RED
                                } else {
                            ...
                            for panel_id in panel_ids.iter().copied() {
                                ...
                                let item_response = ui.horizontal(|ui| {
                                    ...
                                    // Panel type icon (or attention badge)
                                    if has_attention {
                                        ui.label(
                                            egui::RichText::new("!").color(theme::PALETTE_RED).size(12.0).strong(),
                                        );
                                    } else {
                                        let (icon, icon_color) = panel_kind_icon(kind, ws_color, is_focused);
                                        ui.label(
                                ...
                                item_response.response.context_menu(|ui| {
                                    ...
                                    for (other_ws_id, other_ws_name, _, _, _) in &ws_data {
                                    ...
                                    if ui
                                        .add(
                                            Button::new(
                                                egui::RichText::new("Close").size(12.0).color(theme::PALETTE_RED),
                                            )
                                            .frame(false),
                                        )
                                        .clicked()
