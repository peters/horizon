...
impl HorizonBrowserMcp {
    ...
    #[tool(
        ...
        description = "Set or clear HTTP Basic and Digest credentials for a live local Chromium or Firefox panel. This is the only way to supply a username and password for HTTP authentication: MCP clients, CLI run plans, and prompt jobs all call this tool. If you encounter HTTP authentication, call set with the username and password the user provided, and optional origin (http://host[:port] or https://host[:port]), then navigate or reload. The engine provides those credentials once per matching server Basic or Digest challenge; a failed attempt is cancelled instead of looping. Passwords never enter the action audit or Horizon config. Clear drops live-session credentials for later intercepted challenges; it does not revoke Authorization values the browser already cached, so open a new panel for a clean unauthenticated session. Safari and remote device sessions return unsupported_backend."
    )]
