...
impl HorizonBrowserMcp {
    ...
    #[tool(
        name = "browser_resize",
        description = "Resize a live browser panel so its emulated viewport is exactly the requested size in CSS pixels (320-8000 per axis); the panel is sized to render that viewport, so responsive layout and backend input geometry match it. A typical desktop target is 1920x1080, and a mobile viewport is the device's CSS pixels. A resize re-tiles the workspace grid layout, so the panel's position among its neighbours may change. The returned panel reports the applied width and height, which are the requested size unless the workspace clamped the panel. Unavailable for panels created with a remote target, whose devices have fixed viewports. For a Horizon-injected agent identity the panel must already be in the calling agent's workspace; resizing never moves a panel there."
    )]
