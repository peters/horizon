...
impl HorizonBrowserMcp {
    ...
    #[tool(
        ...
        description = "Pause page actions and ask the user to steer in a progress message, then keep this turn active awaiting this tool until they select Done — hand back to agent. Omit timeout_millis for the default 15-minute human wait; do not substitute a 60000 ms page-action timeout. If the client yields a running invocation, keep awaiting that same invocation. Never end the turn with a final waiting message. On timeout, inspect browser_panel once: if handoff_pending is true, call browser_handoff again with resume_request_id from the timeout and the default timeout; if handoff completed, take a fresh snapshot and continue. Also take a fresh snapshot when this tool returns handoff_pending=false. Stop on explicit cancellation, panel closure, lost ownership, or an unrecoverable connection failure. Do not poll browser_list. timeout_millis is 1000-3600000 (default 900000); explicit short waits remain available for scripts/tests. Set wait=false only for an explicitly nonblocking script."
    )]
