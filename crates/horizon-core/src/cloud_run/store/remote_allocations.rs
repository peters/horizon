...
use super::{
    CloudStoreError, CloudWorkflowStore, MAX_RECOVERED_WORKFLOWS, RemoteWorkspaceStoreError as Error,
    StoredRemoteWorkspace, StoredWorkflow, check_recovery_budget, current_unix_millis,
    database::ensure_current_schema,
    encode_workflow, insert_workflow,
...
impl CloudWorkflowStore {
    ...
    /// end after `now_millis`; expiry preserves this relationship for non-creating recovery.
    ...
    pub fn allocate_remote_runtime(
        ...
        ensure_workflow_budget(&transaction, snapshot.len())?;
        let stored_workflow = insert_workflow(&transaction, &workflow, &snapshot)?;
...
fn ensure_workflow_budget(connection: &rusqlite::Connection, next_bytes: usize) -> Result<(), CloudStoreError> {
    ...
    let sizes = statement.query_map(params![current_unix_millis()?, limit], |row| row.get::<_, i64>(0))?;
    let mut count = 1;
