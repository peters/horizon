...
/// Where a browser panel's session runs.
...
fn spawn_browser(
    ...
    let browser = match mode {
        BrowserSpawnMode::Local => {
            crate::browser::BrowserPanelState::start(seed.local_id.clone(), &browser_config, initial_url.clone())?
        }
        BrowserSpawnMode::Remote(request) => crate::browser::BrowserPanelState::start_remote(
            seed.local_id.clone(),
            &browser_config,
            initial_url.clone(),
            *request,
...
fn horizon_codex_mcp_args() -> Vec<String> {
    ...
    vec![
        ...
        // human, up to an hour, so every injected client must outlive that bound.
        "mcp_servers.horizon-browser.tool_timeout_sec=3600".to_string(),
    ]
}
...
mod tests {
    ...
    fn default_codex_launch_registers_only_the_mcp_browser_contract() {
        ...
        assert!(command.contains("mcp_servers.horizon-browser.tool_timeout_sec=3600"));
