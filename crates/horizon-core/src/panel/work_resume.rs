...
impl Panel {
    ...
    pub fn request_work_resume(&mut self) -> Result<()> {
        ...
        if terminal.pending_work_resume().is_none() || !terminal.work_continuation.owns_process {
    ...
    pub(super) fn requested_work_brief(&mut self) -> Result<Option<String>> {
        let Some(requested) = self
            .terminal()
            .and_then(|terminal| terminal.work_continuation.requested_session.as_deref())
        else {
            return Ok(None);
        };
        if self
