...
protocol, and later the draft-plan compiler, routine registry, and
credential-broker interface. It depends on `horizon-browser-protocol` for the
shared action model and on nothing in `horizon-browser`, the MCP adapter, the
CLI runner, or the UI.
