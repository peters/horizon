...
platform supports it. To run on a real remote device, pass `target` with a
configured remote target name and omit `backend`; Horizon resolves the
provider and credentials, and a refusal names the target or credential
reference, never a value. Such a panel reports `remote_target`, classic
WebDriver and no network capture. Set `visible: false` for background automation; use
