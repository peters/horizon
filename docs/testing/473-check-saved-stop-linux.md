...
Status: corrective existing-store adoption after actual #559 squash. The final corrected
candidate, new binary and fresh native receipt are not bound yet; old-head smoke is historical only.
...
- Reuse the reviewed public-only overview harness pattern from
  `/tmp/horizon-473-check-saved-stop-ui-smoke`, in a new
  `/tmp/horizon-473-check-saved-stop-ui-smoke-v2` task-owned namespace.
...
6. Exercise the real UI `execute_check` callback against a genuine compatible v6
   fixture, a current store missing a required index, and an unexpected base-table
   trigger. Assert storage refusal and unchanged committed bytes, user_version and
   journal mode. First retain the failing old-callback regression, then prove the
   corrected callback uses the merged existing-only non-migrating opener. The
   valid-current control must reach the configured-profile error, not storage
   refusal, without reading credentials. Reuse only the pinned workspace SQLite
   library as a UI dev-dependency; no production test API or version upgrade.
7. Missing/insecure store checks must not create or repair the fixture store. A
   successful completion intentionally requires a writer, but Check must not
   initialize or migrate schema, repair permissions, or change journal mode.
   SQLite WAL bookkeeping may occur; do not claim immunity to same-user concurrent
   filesystem tampering.
8. Run fmt, maintainability, version-sync, default/speech workspace tests and all
