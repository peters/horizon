...
export HORIZON_SMOKE_CODEX_HOME="$HORIZON_SMOKE_ROOT/provider-state"
...
{"type":"session_meta","payload":{"id":"review-a","session_id":"review-a","parent_thread_id":"root-a"}}
JSONL
```
...
cp "$HORIZON_SMOKE_RUNTIME" "$HORIZON_SMOKE_ROOT/runtime-last-root-a.yaml"
```
...
metadata whose `session_id` points back to the child, so this proves Horizon
ignores that self-reference and follows `parent_thread_id` to the resumable root,
...
not an already-valid root. Quit and reap the current primary, restore the
captured `resume: last` fixture, rewrite only its isolated binding to
`review-a`, move the database aside again, and launch a new exact primary. The
fixture restore matters because successful automatic repair intentionally pins
the verified root as an explicit resume:

```zsh
wait "$HORIZON_SMOKE_PID"
cp "$HORIZON_SMOKE_ROOT/runtime-last-root-a.yaml" "$HORIZON_SMOKE_RUNTIME"
rg -q 'resume: last' "$HORIZON_SMOKE_RUNTIME"
perl -pi -e 's/session_id: root-a/session_id: review-a/' "$HORIZON_SMOKE_RUNTIME"
rg -q 'session_id: review-a' "$HORIZON_SMOKE_RUNTIME"
mv "$HORIZON_SMOKE_CODEX_HOME/state_5.sqlite" "$HORIZON_SMOKE_ROOT/state_5.sqlite.saved"
...
Select **Open without unverified resumes**. Verify Horizon opens the board and
