...
Require all three to pass. Together they prove an abandoned worker leaves a
terminal fallback with no staged file, the caller alone can publish `running`,
and pre-deadline state JSON still decodes.
