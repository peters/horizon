...
- `cloud_run/store/remote_workspaces.rs` owns validated, session-owned remote
  ...
  board snapshots and session-file deletion. Its prepared replacement operation
  can participate in a larger store transaction without committing independently.
- `cloud_run/store/remote_allocations.rs` binds one workspace generation and one
  explicit single-worker setup workflow atomically, independently of panel count
  or execution lifetime. Its `binding.rs` leaf owns bounded cross-record recovery;
  `guards.rs` prevents generic writes and creation claims from bypassing that
  ownership. Provider lifecycle coordination and runtime-state references remain
